...
static
const char *
    const stop_app = "StopAudioFork";

static
const char *
    const audiofork_spy_type = "AudioFork";
...
struct audiofork {
    ...
    AST_DECLARE_STRING_FIELDS(
        AST_STRING_FIELD(call_context); AST_STRING_FIELD(call_macrocontext); AST_STRING_FIELD(call_extension); AST_STRING_FIELD(call_callerchan); AST_STRING_FIELD(call_callerid);
    );
    int call_priority;
};
...
static
const struct ast_datastore_info audiofork_ds_info = {
    .type = "audiofork",
...
static int setup_audiofork_ds(struct audiofork * audiofork, struct ast_channel * chan, char ** datastore_id,
    const char * beep_id) {
...
static int audiofork_exec(struct ast_channel * chan,
    const char * data) {
    ...
    AST_DECLARE_APP_ARGS(args,
        AST_APP_ARG(wsserver); AST_APP_ARG(options); AST_APP_ARG(post_process);
    );
...
static int stop_audiofork_full(struct ast_channel * chan,
    const char * data) {
    struct ast_datastore * datastore = NULL;
    char * parse = "";
    struct audiofork_ds * audiofork_ds;
...
static int stop_audiofork_exec(struct ast_channel * chan,
    const char * data) {
    ...
    if (!strcasecmp(a - > argv[1], "start")) {
        ...
        AST_LIST_TRAVERSE(ast_channel_datastores(chan), datastore, entry) {
            if (datastore - > info == & audiofork_ds_info) {
                ...
                ast_cli(a - > fd, "%p\t%s\t%s\t%s\n", audiofork_ds, wsserver, filename_read, filename_write);
            }
        }
        ast_channel_unlock(chan);
    } else {
...
static int manager_mute_audiofork(struct mansession * s,
...
static int manager_audiofork(struct mansession * s,
    const struct message * m) {
    struct ast_channel * c;
    const char * name = astman_get_header(m, "Channel");
    const char * id = astman_get_header(m, "ActionID");
...
static int manager_stop_audiofork(struct mansession * s,
    const struct message * m) {
    struct ast_channel * c;
    const char * name = astman_get_header(m, "Channel");
    const char * id = astman_get_header(m, "ActionID");
...
static int func_audiofork_read(struct ast_channel * chan,
    const char * cmd, char * data,
        char * buf, size_t len) {
    struct ast_datastore * datastore;
    struct audiofork_ds * ds_data;
    AST_DECLARE_APP_ARGS(args,
        AST_APP_ARG(id); AST_APP_ARG(key);
    );
...
AST_MODULE_INFO(ASTERISK_GPL_KEY, AST_MODFLAG_DEFAULT, "Audio Forking application",
